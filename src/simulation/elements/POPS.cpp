	...
	if(sim->timer % 500 == 0 && parts[i].tmpcity[5] > 0)
		...
		for (ry = -1; ry < 2; ++ry)
			...
			if (BOUNDS_CHECK && (rx || ry)) {
				r = pmap[y + ry][x + rx];
				int partnum = 10;
				if (!r && parts[i].tmpcity[5] > 0 && parts[i].carbons > 20 && parts[i].hydrogens > 20 && parts[i].water > 20 && parts[i].tmp4 > 20 )
				{
				//	sim->create_part(-1, x + rx, y + ry, PT_OIL);
					int nt = sim->create_part(-1, x + rx, y + ry, PT_WATR);
					parts[nt].hydrogens += 10;
					parts[nt].carbons += 10;
					parts[nt].water += 10;
			...
			//	}
				...
				if (rt == PT_POPS && parts[i].tmpcity[5] > 0 && parts[i].tmpcity[5] > parts[ID(r)].tmpcity[5] && ((parts[ID(r)].tmpcity[0] == 1 && RNG::Ref().chance(1, 2)) || (parts[ID(r)].tmpcity[0] == 2 && RNG::Ref().chance(1, 3)) || (parts[ID(r)].tmpcity[0] == 0 && RNG::Ref().chance(1, 10))))
				{
				...
				/*if (rt == PT_HCL)
				{
					if (parts[ID(r)].tmp4 > 0 && parts[i].carbons < 290 && (sim->elements[parts[ID(r)].ctype].Properties & PROP_EDIBLE || sim->elements[parts[ID(r)].tmpcity[9]].Properties))
					{
