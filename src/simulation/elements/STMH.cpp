...
static int update(UPDATE_FUNC_ARGS) {
	...
	if (surround_space != 0 && parts[i].tmpcity[0] == 0)
		parts[i].tmpcity[0] = 1;
	if (surround_space == 0 && sim->timer % 500 == 0 && parts[i].tmpcity[0] != 0)
		parts[i].tmpcity[0] = 0;
	if(sim->timer % 500 == 0 && parts[i].tmpcity[5] > 0)
		...
		for (ry = -1; ry < 2; ++ry)
			...
			if (BOUNDS_CHECK && (rx || ry)) {
				r = pmap[y + ry][x + rx];
				if (!r && parts[i].tmpcity[5] > 0 && parts[i].hydrogens > 20 && parts[i].oxygens > 20)
				{
					sim->create_part(-1, x + rx, y + ry, PT_HCL);
				...
				//Getting food dissolved in hcl
				if (rt == PT_HCL)
				{
					if (parts[ID(r)].tmp4 > 0 && parts[i].carbons < 290 && sim->elements[parts[ID(r)].ctype].Properties & PROP_EDIBLE  && !(TYP(r) == PT_FLSH && parts[ID(r)].pavg[0] != 2))
					{
					...
					/*if (parts[ID(r)].ctype == PT_SUGR || parts[ID(r)].ctype == PT_SWTR && parts[i].hydrogens < 290)
						...
						if (parts[ID(r)].tmp4 > 0 && parts[i].carbons < parts[i].tmpcity[7] / 2)
						{
							parts[i].carbons += std::min(10, parts[ID(r)].tmp4);
							parts[ID(r)].tmp4 -= std::min(10, parts[ID(r)].tmp4);
						}
						if (parts[ID(r)].carbons > 0 && parts[i].carbons < parts[i].tmpcity[7] / 2)
						{
							parts[i].carbons += std::min(10, parts[ID(r)].carbons);
							parts[ID(r)].carbons -= std::min(10, parts[ID(r)].carbons);
						}
						if (parts[ID(r)].oxygens > 0 && parts[i].oxygens < parts[i].tmpcity[7] / 2)
						{
							parts[i].oxygens += std::min(10, parts[ID(r)].oxygens);
							parts[ID(r)].oxygens -= std::min(10, parts[ID(r)].oxygens);
						}
						if (parts[ID(r)].hydrogens > 0 && parts[i].hydrogens < parts[i].tmpcity[7] / 3)
						{
							parts[i].hydrogens += std::min(10, parts[ID(r)].hydrogens);
							parts[ID(r)].hydrogens -= std::min(10, parts[ID(r)].hydrogens);
						}
						if (parts[ID(r)].water > 0 && parts[i].water < parts[i].tmpcity[7] / 3)
						{
							parts[i].water += std::min(10, parts[ID(r)].water);
							parts[ID(r)].water -= std::min(10, parts[ID(r)].water);
						}
						if (parts[ID(r)].tmp4 <= 0 && parts[ID(r)].hydrogens <= 0 && parts[ID(r)].oxygens <= 0 && parts[ID(r)].carbons <= 0 && parts[ID(r)].water <= 0 && RNG::Ref().chance(1, 10))
							sim->kill_part(ID(r));
						}
					if(parts[ID(r)].tmp4 <= 0 && parts[ID(r)].hydrogens <= 0 && parts[ID(r)].oxygens <= 0 && parts[ID(r)].carbons <= 0 && parts[ID(r)].water <= 0 && RNG::Ref().chance(parts[ID(r)].tmpcity[2], 100000))
						sim->kill_part(ID(r));
