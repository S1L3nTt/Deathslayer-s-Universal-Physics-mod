...
void Element::Element_BVSL() {
	...
	HeatConduct = 250;
	Description = "Blood vessel. Transports nutrients, carbon and oxygen.";
				...
				if (BOUNDS_CHECK && (rx || ry)) {
					...
					if (rt == PT_FLSH || rt == PT_STMH || rt == PT_UDDR || rt == PT_LUNG || rt == PT_POPS)
					{
						...
						if (RNG::Ref().chance(1, 8) && capacity + partnum < parts[i].tmpcity[7])
						{
							...
							if (((rt == PT_LUNG && parts[ID(r)].hydrogens > parts[i].tmpcity[7] / 3 && parts[i].hydrogens + partnum < parts[i].tmpcity[7] / 3) || (rt != PT_LUNG && parts[i].hydrogens + partnum > parts[i].tmpcity[7] / 3)) && parts[ID(r)].hydrogens >= partnum + 10 && parts[ID(r)].hydrogens > parts[i].hydrogens)
							{
								parts[i].hydrogens += partnum;
								parts[ID(r)].hydrogens -= partnum;
...
//	if (cpart->tmp2 < CLOT)
	...
	*colg *= 0.2f + 0.8f * cpart->hydrogens / 100.0f;
	*colb *= 0.2f + 0.8f * cpart->carbons / 100.0f;
